...
export default function TikTokDMViewer() {
  ...
  function handleFileUpload(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    reader.onload = (ev) => {
      try {
        ...
        const chats = data?.["Direct Message"]?.["Direct Messages"]?.["ChatHistory"];
  ...
  const inboxList = useMemo(() => {
    if (!messages || filtered.length === 0) {
      return <p className="text-gray-500 mt-6 text-center">No messages found</p>;
    }
    return filtered.map(([u, c], i) => (
      <div key={i} className="flex items-center px-4 py-3 hover:bg-gray-100 cursor-pointer" onClick={() => setSelectedChat(u)}>
        ...
        <div className="flex-1 min-w-0">
          ...
          <p className="text-gray-500 text-sm truncate">{c[c.length - 1]?.Content ?? "No content"}</p>
        </div>
        <span className="text-xs text-gray-400">{formatDate(c[c.length - 1]?.Date)}</span>
      </div>
    ));
  }, [filtered, messages]);
    ...
    return (
      <div className="fixed inset-0 flex flex-col w-full h-full max-w-md mx-auto z-50 bg-[#f8f8f8]">
        <div className="relative h-14 w-full flex items-center justify-center px-4 bg-[#f8f8f8] border-b border-gray-200">
          <button onClick={() => setSelectedChat(null)} className="absolute left-4 text-black text-2xl">
            &lt;
          </button>
          ...
          {arr.map((m, i) => {
            ...
            return (
              <div key={i} className={`flex items-end gap-2 ${isUser ? "justify-end" : "justify-start"}`}>
                ...
                <div className={`max-w-[75%] px-4 py-2 rounded-2xl text-sm break-words whitespace-pre-wrap ${isUser ? "bg-[#1092d6] text-white self-end" : "bg-white text-black"}`}>
                  {link ? (
                    <div>
                      ...
                      <p className="text-xs break-words whitespace-pre-wrap">{m.Content}</p>
                    </div>
                  ) : (
                    <span className="break-words whitespace-pre-wrap">{m.Content}</span>
                  )}
                  <span className={`block text-xs mt-1 ${stamp}`}>{formatDate(m.Date)}</span>
                </div>
              </div>
            );
          })}
    ...
    <div className="min-h-screen bg-white text-black flex flex-col items-center w-full max-w-md mx-auto relative">
      {!selectedChat && (
        <div className="relative h-14 w-full border-b border-gray-200">
          <div className={`absolute inset-0 flex items-center transition-all duration-300 ${isSearching ? "opacity-0 pointer-events-none" : "opacity-100"}`}>
            ...
            <div className="absolute right-4">
              <svg onClick={() => setIsSearching(true)} className="w-6 h-6 text-gray-500 cursor-pointer" fill="currentColor" viewBox="0 0 24 24">
                <path d="M10 2a8 8 0 105.29 14.012l4.7 4.7a1 1 0 001.42-1.42l-4.7-4.7A8 8 0 0010 2zM4 10a6 6 0 1112 0 6 6 0 01-12 0z" />
              </svg>
            </div>
          </div>
          <div className={`absolute inset-0 px-4 flex items-center transition-all duration-300 ${isSearching ? "opacity-100" : "opacity-0 pointer-events-none"}`}>
            <input
              type="text"
      ...
      {!selectedChat && <div className="flex-1 w-full overflow-y-auto">{inboxList}</div>}
      {chatView}
      {showModal && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex justify-center items-center z-50">
          <div className="bg-white p-6 rounded-lg shadow-lg text-center">
            <h2 className="text-lg font-semibold mb-4">Upload Your TikTok DM JSON File</h2>
            <div className="border-2 border-dashed p-6 rounded-lg cursor-pointer hover:bg-gray-100 relative">
              <input type="file" className="absolute inset-0 opacity-0 w-full h-full cursor-pointer" onChange={handleFileUpload} />
